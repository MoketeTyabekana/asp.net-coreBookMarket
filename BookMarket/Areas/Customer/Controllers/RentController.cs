...
namespace BookMarket.Areas.Customer.Controllers
{
    ...
    public class RentController : Controller
    {
        ...
        public RentController(ApplicationDbContext db, UserManager<ApplicationUser> userManager, IHostingEnvironment he)
        {
            _db = db;
            _he = he;
        ...
        public async Task<IActionResult> Rent(RentApplication application, IFormFile ID, IFormFile Address, IFormFile NxtID)
        {
            ...
            if (NxtID != null)
            {
                var next = Path.Combine(_he.WebRootPath + "/files", Path.GetFileName(NxtID.FileName));
                await NxtID.CopyToAsync(new FileStream(next, FileMode.Create));
                application.ProofOfAddress = "files/" + NxtID.FileName;
            }
            applicationDetails.UserId = _userManager.GetUserId(User);
            application.Status = "Pending";
            application.RentApplicationDetails.Add(applicationDetails);

            _db.RentApplication.Add(application);
